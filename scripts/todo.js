...
function hienThiTask() {
    ...
    hienThiTatCaBaiViet(filterTask);
};
...
function hienThiTatCaBaiViet(todoListArr) {
    let html = '';
    //sử dụng vòng lặp forEach để lặp quá các phần tử của mảng
    todoListArr.forEach(x => {
        if (x.isDone) { 
            html += `<li class="checked" onclick="toggleTask('${x.task}', ${x.isDone})"> ${x.owner}<span class="close" onclick="deleteTask(event, '${x.task}', ${x.isDone}, '${x.owner}')">×</span></li>`;
        } else {
            html += `<li onclick="toggleTask('${x.task}', ${x.isDone})"> ${x.task}<span class="close" onclick="deleteTask(event, '${x.task}', ${x.isDone}, '${x.owner}')">x</span></li>`;
        }
